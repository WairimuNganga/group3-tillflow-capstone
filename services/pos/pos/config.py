...
def _url_from_credentials(raw: str) -> str:
    """Async URL from the ECS-injected ``pos`` entry of the ``devops-g3/db`` secret
    (``{username, password, host, port, dbname, sslmode}``). RDS Proxy has
...
class Settings(BaseSettings):
    """POS service configuration — secrets come from env / Secrets Manager."""
    ...
    service_name: str = Field(default="pos", alias="SERVICE_NAME")
    git_commit_sha: str = Field(default="unknown", alias="GIT_COMMIT_SHA")
    log_level: str = Field(default="INFO", alias="LOG_LEVEL")

    # Runtime connection: the NOBYPASSRLS role `tillflow_pos`. Omit to run the
    # service on in-memory stores (local / CI without Postgres).
    database_url: str = Field(default="", alias="DATABASE_URL")

    # In ECS the runtime credentials arrive as JSON (`DB_CREDENTIALS`, the `pos`
    # key of `devops-g3/db`) rather than a URL; DATABASE_URL is derived from it.
    db_credentials: str = Field(default="", alias="DB_CREDENTIALS", repr=False)

    # Migration/admin connection: a role that is a member of `tillflow_pos_owner`
    # (RDS master in AWS; the superuser locally). Alembic uses this and switches
    # to the owner role, so migrated objects are owned by the owner and RLS +
    # default-privilege grants apply (ADR-005 / G2 handover). Falls back to
