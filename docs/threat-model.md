...
- **Status:** **G0 approved**  Four Platform corrections from that
  review are applied below (T2.1/T3.2 routing, T4.6 RDS Proxy, T4.4/T8.4 ALB log encryption,
  T2.4/AR-7 egress). Later-gate amendments remain open against [ADR-004](adr/ADR-004-idempotency-and-money-integrity.md)
  and [ADR-005](adr/ADR-005-multi-tenancy-isolation.md) — see Production release blockers.
- **DRI:** Lwam
- **Date:** 2026-09-08
...
- WAF/rate-limit rules, tenant isolation, least-privilege IAM, egress restriction, backup restore,
  ...
  document;
- outbound HTTPS egress from `payments` and the G2 database bootstrap job is unrestricted and
  accepted, not controlled (AR-7); a domain-aware egress control is required before launch, and
  there is no account-level audit trail yet (T6.6);
...
| # | STRIDE | Threat | Mitigation | Owner | Proof |
|---|---|---|---|---|---|
| T2.1 | Spoofing | Something inside the VPC calls the ALB directly, bypassing the gateway's authn/throttling | **The ALB carries external traffic only.** Its SG accepts inbound 443 exclusively from the VPC Link ENIs, and per-service SGs accept ALB traffic only on that service's port. Internal service-to-service calls do **not** traverse the ALB at all — they use ECS Service Connect (see T3.2), so there is no legitimate in-VPC caller of the ALB to carve an exception for | Lwam | SG rules in `terraform plan` |
...
| T2.4 | Information disclosure | **Outbound exfiltration** — a compromised task, build job, or malicious dependency uses the shared NAT egress path to send tenant data, MSISDNs, or Daraja credentials to an attacker-controlled host. [ADR-010](adr/ADR-010-networking-topology.md) is explicit about inbound security groups and silent about egress, so today's design permits arbitrary outbound HTTPS from selected app-subnet workloads | Reduce egress as far as the primitives actually allow, and be honest about the remainder. **Security groups filter by CIDR and prefix list, not by hostname** — Daraja publishes no stable IP range or AWS prefix list, so a rule that genuinely means "Daraja only" is not expressible with SGs. What *is* implementable and is in scope: VPC endpoints for S3, ECR, Secrets Manager, SSM, SQS, KMS, and CloudWatch so AWS-service traffic stays private where supported; per-service SG egress rules that deny everything except 443 for the one runtime service (`payments`) that needs internet access, with `web`, `pos`, and `commission` given no internet egress path; and a separate G2 `db-bootstrap` CodeBuild SG with 443 egress for the one-off bootstrap job that installs the PostgreSQL client and reaches AWS APIs. Compensating controls are least-privilege task/build roles, no user-controlled fetch URLs (T2.5), VPC Flow Logs on the app subnets, and alerts on unexpected destinations. The residual — unrestricted HTTPS from `payments` and build-time HTTPS from `db-bootstrap` to any host — is **accepted as AR-7**, not claimed as controlled. A domain-aware egress control (AWS Network Firewall or an explicit forward proxy) is the upgrade path if that risk is revisited | Lwam | SG egress rules + VPC endpoint list in `terraform plan`; flow-log sample with the unexpected-destination alert |
...
| T3.2 | Spoofing | Unauthorized internal caller invokes `payments`' B2C endpoint | `commission → payments` runs over **ECS Service Connect**, not the ALB, so the internal call path is separate from the external one (T2.1) and the B2C route is never reachable through API Gateway. Network reachability is restricted by SG to the calling service's SG, but the network is **not** the authentication boundary: the caller presents a verifiable service identity that `payments` validates at the application layer before acting. A route that moves money is not authorized by source IP | Hunter | route map + SG rules + a test showing an SG-reachable caller without a valid service identity is rejected |
...
| T4.4 | Information disclosure | Data readable at rest by anyone with storage access | SSE-KMS (shared CMK, rotation on) on every bucket **except the ALB access-log bucket**, BPA at bucket **and** account level, RDS storage encryption, Redis encryption in transit + at rest ([ADR-003](adr/ADR-003-object-storage.md)). **Documented exception:** ALB access-log delivery does not support SSE-KMS with a customer-managed key, so that bucket uses SSE-S3 (AES256) on a dedicated bucket rather than sharing the KMS-encrypted logs bucket — see T8.4, recorded in ADR-003 and [`scar-log.md`](scar-log.md) | Lwam | bucket policies + RDS config |
...
| T4.6 | Denial of service | One tenant's or one service's runaway query starves the shared RDS instance | **RDS Proxy does not provide a per-service connection quota** — `MaxConnectionsPercent` sizes the proxy's own target-pool ceiling against the instance, it is not a per-credential limit, so the proxy alone cannot stop one service monopolising connections. Containment comes from the application side instead: a bounded connection pool per service (max size set in config, not left to driver defaults), `statement_timeout` and `idle_in_transaction_session_timeout` on every service role so a runaway query is killed rather than held, and proxy headroom left below the instance maximum. Detection is a **connection-borrow latency** alarm — the signal that one consumer is starving the others. Single-instance blast radius remains a **known accepted risk** (AR-2 below) | Lwam | pool + timeout settings in `terraform plan` and service config; borrow-latency alarm |
...
| T6.7 | Spoofing / Tampering | **Adapter and environment confusion.** [ADR-007](adr/ADR-007-m-pesa-adapter.md) selects the M-Pesa adapter from configuration (`MPESA_ADAPTER=sandbox\|fake`). A misapplied environment variable could run `FakeMpesaAdapter` in a deployed environment, causing sales to settle without provider-confirmed money movement. The reverse error, a deployed environment pointed at another environment's credentials or callback URL, cross-contaminates payment state between environments. The release-blocker list already forbids this; it needs a control, not just a prohibition | The adapter is resolved at startup and **fails closed**: any deployed environment refuses to boot if the adapter is not `sandbox`, rather than defaulting. The resolved adapter, environment name, commit SHA, and image digest are exposed at `/health` and asserted by the post-deploy smoke check ([ADR-009](adr/ADR-009-ci-cd-promotion-and-rollback.md)), so a wrong adapter fails the deploy gate instead of quietly serving traffic. Callback URLs and credentials are environment-scoped secrets, never shared across environments | Wairimu / Hunter | `/health` output showing the resolved adapter + a smoke check that fails on a deliberately wrong adapter |
...
| T7.3 | Spoofing | Ambiguity about which artifact is actually running | No `latest` tag ever; build by commit SHA, deploy the **immutable digest**, expose commit + digest at runtime (`/health`) and in pipeline evidence | Wairimu | runtime `/health` output vs pipeline log |
...
| T8.4 | Repudiation | ALB access logs, the audit trail for edge traffic, are missing or mutable | Enable ALB access logging to a **dedicated** bucket, `devops-g3-alb-logs-<account_id>`, kept private, versioned, BPA-blocked, and retained per [ADR-003](adr/ADR-003-object-storage.md). It uses **SSE-S3 (AES256), not SSE-KMS** — ALB log delivery does not support a customer-managed KMS key, so forcing the shared CMK would silently stop log delivery and destroy the audit trail this row exists to protect. Isolating it in its own bucket keeps the KMS requirement intact everywhere else instead of weakening the shared logs bucket to accommodate one producer. Deviation recorded in ADR-003 and [`scar-log.md`](scar-log.md) | Lwam | bucket policy showing SSE-S3 + sample log objects proving delivery works |
...
| AR-7 | **Unrestricted HTTPS egress from `payments` and build-time HTTPS egress from `db-bootstrap`** (T2.4) — security groups cannot filter by hostname, and Daraja publishes no stable IP range or AWS prefix list, so "Daraja-only egress" is not expressible with the primitives available. The `db-bootstrap` path is not a runtime service path; it is a Terraform-managed CodeBuild job used to create schemas, roles, and DB credentials after the dev stack exists. Compensating controls: no internet egress for `web`, `pos`, or `commission`; VPC endpoints for AWS services; least-privilege task/build roles; no user-controlled fetch URLs (T2.5); and flow-log alerting on unexpected destinations | A domain-aware egress control (AWS Network Firewall, or a forward proxy the tasks/build jobs are forced through) adds cost and operational surface for a sandbox-only workload | Lwam | Re-evaluate at G3 if flow logs show unexpected destinations; required before any production launch (listed as a release blocker) |
...
6. **PII redaction** — a captured log line and span for a sandbox payment showing masked MSISDNs.
   All evidence uses Daraja sandbox and **synthetic** MSISDNs only; no real subscriber number
   appears in any artifact (`evidence/reliability/`).
...
9. **Egress reduction** — `terraform plan` showing VPC endpoints for S3/ECR/Secrets Manager/
   CloudWatch, no internet egress path for `pos`/`web`/`commission`, 443-only egress for
   `payments`, and 443-only build-time egress for `db-bootstrap`; plus a VPC Flow Logs sample and
   the unexpected-destination alert. The residual any-host HTTPS egress is recorded as AR-7, not
   claimed as controlled (T2.4).
...
25. **Adapter fail-closed** — `/health` exposing the resolved adapter, environment, commit SHA, and
