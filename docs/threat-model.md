...
- WAF/rate-limit rules, tenant isolation, least-privilege IAM, egress restriction, backup restore,
  ...
  document;
- outbound HTTPS egress from `payments` is unrestricted and only accepted, not controlled (AR-7);
  a domain-aware egress control is required before launch, and there is no account-level audit
  trail yet (T6.6);
...
| T2.4 | Information disclosure | **Outbound exfiltration** — a compromised task or malicious dependency uses the shared NAT egress path to send tenant data, MSISDNs, or Daraja credentials to an attacker-controlled host. [ADR-010](adr/ADR-010-networking-topology.md) is explicit about inbound security groups and silent about egress, so today's design permits arbitrary outbound HTTPS from the app subnets | Reduce egress as far as the primitives actually allow, and be honest about the remainder. **Security groups filter by CIDR and prefix list, not by hostname** — Daraja publishes no stable IP range or AWS prefix list, so a rule that genuinely means "Daraja only" is not expressible with SGs. What *is* implementable and is in scope for G1: VPC endpoints for S3, ECR, Secrets Manager, and CloudWatch so AWS-service traffic never traverses NAT at all; per-service SG egress rules that deny everything except 443 for the one service (`payments`) that needs the internet, with the other three services given no internet egress path whatsoever; least-privilege task roles so a compromised task holds little worth exfiltrating; no user-controlled fetch URLs (T2.5); VPC Flow Logs on the app subnets with an alert on unexpected destinations. The residual — unrestricted HTTPS from `payments` to any host — is **accepted as AR-7**, not claimed as controlled. A domain-aware egress control (AWS Network Firewall or an explicit forward proxy) is the upgrade path if that risk is ever revisited | Lwam | SG egress rules + VPC endpoint list in `terraform plan`; flow-log sample with the unexpected-destination alert |
...
| AR-7 | **Unrestricted HTTPS egress from `payments` to any internet host** (T2.4) — security groups cannot filter by hostname, and Daraja publishes no stable IP range or AWS prefix list, so "Daraja-only egress" is not expressible with the primitives available. Compensating controls: no internet egress at all for the other three services, VPC endpoints for AWS services, least-privilege task roles, no user-controlled fetch URLs (T2.5), and flow-log alerting on unexpected destinations | A domain-aware egress control (AWS Network Firewall, or a forward proxy the tasks are forced through) is real cost and real operational surface for a sandbox-only workload | Lwam | Re-evaluate at G3 if flow logs show unexpected destinations; required before any production launch (listed as a release blocker) |
...
9. **Egress reduction** — `terraform plan` showing VPC endpoints for S3/ECR/Secrets Manager/
   CloudWatch, no internet egress path for `pos`/`web`/`commission`, and 443-only egress for
   `payments`; plus a VPC Flow Logs sample and the unexpected-destination alert. The residual
   any-host HTTPS egress is recorded as AR-7, not claimed as controlled (T2.4).
